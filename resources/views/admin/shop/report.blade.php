    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
            ...
            <p class="text-xl font-black text-slate-900 mt-1 truncate">
                {{ $bestSellers->first()->product->name ?? 'Chưa có dữ liệu' }}
            </p>
        </div>
    </div>
            ...
            <div class="space-y-6">
                ...
                <div class="flex items-center justify-between group">
                    <div class="flex items-center space-x-3 w-[85%]">
                        <div class="w-8 h-8 rounded-lg bg-slate-800 flex items-center justify-center text-[10px] font-black group-hover:bg-pink-600 transition-colors flex-shrink-0">
                            {{ $loop->iteration }}
                        </div>
                        <div class="truncate">
                            <p class="text-xs font-bold leading-tight truncate" title="{{ $item->product->name ?? '' }}">
                                {{ $item->product->name ?? 'Sản phẩm không tồn tại' }}
                            </p>
                            <p class="text-[9px] text-gray-400 uppercase mt-1">Đã bán: <span class="text-pink-400 font-bold">{{ $item->sold_count }}</span> sản phẩm</p>
                        </div>
                    </div>
                ...
                <div class="text-center py-8 text-gray-500 text-xs italic">
                    Chưa phát sinh sản phẩm bán ra.
                </div>
                @endforelse
            </div>
        </div>
