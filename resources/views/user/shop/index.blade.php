        ...
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
                ...
                <div class="flex items-center bg-gray-50 p-1.5 rounded-2xl border border-gray-100">
                    <span class="px-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Danh mục</span>
                    <form action="{{ route('shop.index') }}" method="GET">
                        <input type="hidden" name="branch_id" value="{{ request('branch_id') }}">
                        <select name="category_id" onchange="this.form.submit()" class="bg-white border-none rounded-xl text-sm font-bold py-2.5 px-6 outline-none shadow-sm cursor-pointer">
    ...
    <div class="bg-white border-b overflow-x-auto custom-scrollbar">
        <div class="max-w-7xl mx-auto px-4 py-8">
            ...
            <div class="flex space-x-8">
                ...
                <a href="{{ route('shop.index', ['branch_id' => request('branch_id'), 'partner_id' => $partner->id]) }}" class="flex-shrink-0 flex flex-col items-center group">
                    <div class="w-16 h-16 rounded-2xl border-2 {{ request('partner_id') == $partner->id ? 'border-[#a61d6d] bg-pink-50' : 'border-gray-100' }} flex items-center justify-center overflow-hidden transition-all group-hover:scale-105 bg-white">
                        @if($partner->avatar)
                            <img src="{{ asset('storage/' . $partner->avatar) }}" class="w-full h-full object-cover">
                        @else
                            <span class="text-xs font-black text-gray-300 italic">{{ substr($partner->name, 0, 2) }}</span>
                        @endif
                    </div>
                    <span class="text-[10px] font-bold mt-3 uppercase tracking-tight {{ request('partner_id') == $partner->id ? 'text-[#a61d6d]' : 'text-gray-500' }}">{{ $partner->name }}</span>
                </a>
                @endforeach
            </div>
        </div>
    ...
    <main class="max-w-7xl mx-auto px-4 py-16">
        ...
        @else
            <div class="bg-white rounded-[3rem] p-24 text-center border-2 border-dashed border-gray-100">
                ...
                <h3 class="text-2xl font-black text-gray-800 italic uppercase">Hết hàng rồi Đạt ơi!</h3>
